...
# # 定义模拟蒸发过程的函数
# def model_evaporation(X, k1, k2, k3, k4, kh, dt):
#     T, H, SC = X
#     results = []
...
#             phi_c = m_C_out / p_C / (m_C / p_C + m_D / p_D + m_S / p_S)    # 固体粒子体积分数
            
#             # 计算浓度
#             V = m_S/p_S + m_D/p_D
#             c = (m_S + m_D) / V

#             # 计算饱和蒸汽压 (简化公式，实际应用中可能需要更复杂的计算)
#             P_sat = np.exp(6.09451 - 2725.96 / temp + 28.209)  # 简化公式
            
#             # 更新液滴质量
#             dm_D_dt = -k1 * P_sat * c * (1 - (humidity * kh ) / 100)  
#             m_D += dm_D_dt * dt
...
#             # 计算扩散系数
#             D = (k_B * temp) / (6 * np.pi * eta * r)

#             # 计算平均速度
#             v_avg = np.sqrt(6 * D / dt)

#             # 计算 Z
#             Z = np.sqrt(2 * n * np.pi * r**2 * v_avg)

#             # 更新输出质量
#             dm_S_out_dt = np.maximum(m_S - m_D * S_S, 0)
#             m_S_out += dm_S_out_dt * dt

#             dm_C_out_dt = np.maximum(m_C - m_D * S_C, 0)
#             m_C_out += dm_C_out_dt * dt

#             # 更新形成和生长质量
#             dm_form_dt = k2 * Z
#             m_form += dm_form_dt * dt

#             c_small = m_S_out / (n * np.pi * r ** 3)

#             dm_grow_dt = k3 * v_avg * c_small * (m_S_out ** (2/3))
#             m_grow += dm_grow_dt * dt

#             # 计算孔隙率
#             integral += (m_form + m_grow) * dt
...
# # 拟合模型（初始猜测）
# dt = 1.0
# popt, pcov = curve_fit(model_evaporation, X_data, y_data, p0=[10., 20., 10., 20., 0.5, dt])
# k1_fit, k2_fit, k3_fit, k4_fit, kh_fit, dt_fit = popt

# # 预测与评估
# y_pred = model_evaporation(X_data, k1_fit, k2_fit, k3_fit, k4_fit, kh_fit, dt)
